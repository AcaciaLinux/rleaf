use crate::rleaf_sys::{cleaf_finalize, cleaf_init};
...
pub fn add_leaf_instance() {
    ...
    trace!(
        "[rleaf] Registering new leaf instance, {} in total now",
        LEAF_INSTANCES.load(Ordering::Relaxed)
    );
}
...
pub fn remove_leaf_instance() {
    ...
    trace!(
        "[rleaf] Unregistering a leaf instance, {} remaining",
        LEAF_INSTANCES.load(Ordering::Relaxed)
    );
